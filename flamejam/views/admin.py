...
from flamejam.models import User, Jam, JamPhoto, Game
from flamejam.forms import JamDetailsForm, JamPhotoForm, AdminWriteAnnouncement, AdminUserForm
...
@app.route("/admin/jams/photos/<int:id>", methods=["POST", "GET"])
@admin_permission.require()
def admin_jam_photos(id):
    jam = Jam.query.filter_by(id=id).first_or_404()
    photos = JamPhoto.query.filter_by(jam_id=id).all()

    form = JamPhotoForm(obj=jam)
    if request.method == "POST":
        # if form.validate_on_submit():
        photo = JamPhoto(jam.id, form.photo)
        photo.photo = request.files["photo"].read()
        db.session.add(photo)
        db.session.commit()
        flash("Jam photo has been saved.", "success")
        return redirect(url_for("admin_jam_photos", id=jam.id))

    return render_template("admin/jam_photos.html", photos=photos, jam=jam, form=form)


@app.route("/jams/photos/<int:id>/delete", methods=["GET"])
@admin_permission.require()
def admin_jam_photo_delete(id):
    photo = JamPhoto.query.filter_by(id=id).first_or_404()
    jam_id = photo.jam_id
    db.session.delete(photo)
    db.session.commit()

    return redirect(url_for("admin_jam_photos", id=jam_id))


@app.route("/jams/photos/<int:id>", methods=["GET"])
def jam_photo(id):
    photo = JamPhoto.query.filter_by(id=id).first_or_404()

    return app.response_class(photo.photo, mimetype='image/jpeg')
